...
volatile new_dma_ctrl_table dma_alte_ctrl_table[32];
...
void ADC0_FIFO0_setup()
{
    ...
    */
    ...
    GPIO_PORTB_AFSEL_R = 0x10;          // Enables alternate function on PB4

    GPIO_PORTB_DEN_R |= 0x10;           // Sets PB4 to digital function
     ...
     * Sample Sequencer 0
     * FIFO = 8
     * PB4 = AIN10 = 0d10 = 0b1010 = 0xA
     * inputs = 0x AAAA AAAA
     * PWM Gen. 0 , Module 0
     *
     */

    ADC0_ACTSS_R &= ~1;             // 1. Disable SS0 for setup

    ADC0_EMUX_R = (0x6);            // 2. Sets triggerevent to PWM0

    ADC0_TSSEL_R &= ~(3 << 4);      // 3. Specify PWM Gen. 0, Module 0

    ADC0_SSMUX0_R = 0xAAAAAAAA;     // 4. Specify input sources for samples, AIN10, PB4 for all

    ADC0_SSCTL0_R |= (1 << 29);     // 5. Specify end bit - 8th sample is end of sequence

    ADC0_IM_R |= 1;                 // 6. Enable MASK bit for SS0, interrupt

    ADC0_ACTSS_R = 1;               // 7. Enable the SS0 again.

    ADC0_ISC_R = 1;                 // Clear interrupt flag
}
