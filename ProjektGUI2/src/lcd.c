...
{
    GPIO_PORTC_DATA_R &= ~(0xF0);               // Makes sure that the bits are turned off.
    GPIO_PORTC_DATA_R |= ( value );             //T�nder for de �vre bits
    pulse_enable();                             // Sender en puls p� enable for at sende bits
    GPIO_PORTC_DATA_R &= ~( 0xF0 );             // S�ger for at de bliver slukket igen

    GPIO_PORTC_DATA_R |= (value << 4);          //T�nder for de �vre bits
    pulse_enable();                             // Sender en puls p� enable for at sende bits
    GPIO_PORTC_DATA_R &= ~( 0xF0 );             // S�ger for at de bliver slukket igen
